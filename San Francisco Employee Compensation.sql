...
# Data Exploration
---------------------------------------------------------------------------------------------
# Skills Used: Joins, Subqueries, Window Functions, Case When, Aggregated Functions
---------------------------------------------------------------------------------------------
...
# Percentage of Employees with Below Average Salary each Year
-- Temp Table to calculate the average salary for each year

CREATE TEMPORARY TABLE YearlyAvgSalary AS 
	SELECT Year, AVG(Total_Salary) AS AvgSal
    FROM employee
    GROUP BY year;
...
SELECT e.year, 
    ...
    AvgSal AS 'Yearly Avg Salary',
    SUM(CASE WHEN Total_Salary < AvgSal THEN 1 ELSE 0 END) AS 'Employees Below Avg Salary',
    (SUM(CASE WHEN Total_Salary < AvgSal THEN 1 ELSE 0 END) / COUNT(*)) * 100 AS 'Percentage of Emp. Below Avg. Salary'
FROM employee AS e
INNER JOIN YearlyAvgSalary AS y
ON e.year = y.year
GROUP BY year, AvgSal
ORDER BY year DESC;

SELECT AVG(`Percentage of Emp. Below Avg. Salary`)
FROM one;

DROP TEMPORARY TABLE YearlyAvgSalary;


# Analyzing Total Salary over the Past Decade with Window Functions
-- Shows the Percentage Change in Salary each Year

CREATE TEMPORARY TABLE RateofChangeYearly AS
...
# Then Analyzing Average Rate of Change of Past Decade
SELECT
	AVG(`Rate of Change %`) 
FROM RateofChangeYearly;

# Analyzing Average Rate of Change since 2019
SELECT
	AVG(`Rate of Change %`) 
FROM RateofChangeYearly
WHERE year > 2019 ;

DROP TEMPORARY TABLE RateofChangeYearly;